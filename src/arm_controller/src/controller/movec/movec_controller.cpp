...
void MoveCController::command_queue_consumer_thread() {
    ...
    std::vector<std::future<void>> in_flight_tasks;

    auto cleanup_finished_tasks = [&in_flight_tasks]() {
        in_flight_tasks.erase(
            std::remove_if(in_flight_tasks.begin(), in_flight_tasks.end(),
            ...
            try {
                if (state_mgr) {
                    ...
                    if (state_mgr->isInHookState()) {
                        ...
                        RCLCPP_DEBUG(node_->get_logger(), "[%s] 🛑 MoveC in hook state - requesting transition to %s",
                        ...
                        RCLCPP_INFO(node_->get_logger(),
                                    "[%s] MoveC command deferred during hook transition, re-queued (ID: %s)",
