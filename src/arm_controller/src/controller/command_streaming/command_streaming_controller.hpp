#ifndef __COMMAND_STREAMING_CONTROLLER_HPP__
#define __COMMAND_STREAMING_CONTROLLER_HPP__
...
class CommandStreamingController final
    : public VelocityControllerImpl<sensor_msgs::msg::JointState> {
public:
    explicit CommandStreamingController(const rclcpp::Node::SharedPtr& node);
    ~CommandStreamingController() override;
...
private:
    ...
    JointCommandData normalize_joint_command(
        const std::string& mapping,
        const JointCommandData& input) const;
    JointCommandData decode_packed_command(
        const std::string& mapping,
        const std::vector<double>& packed_command) const;

    bool send_joint_status(const std::string& mapping, 
...
#endif      // __COMMAND_STREAMING_CONTROLLER_HPP__
