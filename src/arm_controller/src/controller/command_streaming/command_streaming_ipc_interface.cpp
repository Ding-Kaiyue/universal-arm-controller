#include "command_streaming_ipc_interface.hpp"
...
bool CommandStreamingIPCInterface::execute(const std::vector<double>& packed_command,
    ...
    val_result = ipc::CommandValidator::validateCommandStreaming(packed_command);
    if (!val_result.valid) {
        setLastError(val_result.error_message);
        return false;
    }
    ...
    auto cmd = ipc::CommandBuilder()
        .withMode("CommandStreaming")
        .withMapping(mapping)
        .withJointVelocities(packed_command)
        .build();
...
std::string CommandStreamingIPCInterface::getCurrentMode(const std::string& mapping) const {
    if (!ensureInitialized()) {
        return "";
    }
...
ipc::ExecutionState CommandStreamingIPCInterface::getExecutionState(const std::string& mapping) const {
    if (!ensureInitialized()) {
