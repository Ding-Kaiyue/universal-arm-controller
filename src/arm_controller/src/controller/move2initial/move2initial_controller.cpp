...
bool Move2InitialController::execute_trajectory(
    ...
    try {
        ...
        if (interface.empty()) {
            RCLCPP_ERROR(node_->get_logger(), "[%s] ❎ Move2Initial: No interface found for mapping", mapping.c_str());
            return false;
        }

        // 执行轨迹
        if (!hardware_manager_->executeTrajectory(interface, trajectory)) {
            RCLCPP_ERROR(node_->get_logger(), "[%s] ❎ Move2Initial: Failed to execute trajectory on interface: %s",
